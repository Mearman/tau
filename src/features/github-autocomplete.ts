/**
 * GitHub issue autocomplete feature — preloads open issues from the
 * current GitHub repo and provides # triggered autocomplete.
 */
...
async function resolveGitHubRepo(
    ...
    if (result.code !== 0) {
        return {
            ok: false,
            error: "github-issue-autocomplete: cwd is not a git repository",
        };
    }
    ...
    return {
        ok: false,
        error: "github-issue-autocomplete: cwd is not a GitHub repository",
    };
}
            ...
            issuesPromise ||= (async () => {
                ...
                if (result.code !== 0) {
                    if (!loadErrorShown) {
                        ...
                        ctx.ui.notify(
                            `github-issue-autocomplete: failed to load issues: ${details}`,
                            "error"
                        );
                    }
                    return undefined;
