...
export default function (pi: ExtensionAPI) {
    ...
    /** Job ID awaiting an agent decision after timeout. */
    let pendingDecisionJobId: string | undefined;
    ...
    function startTitlebarSpinner(ctx: {
        ui: { setTitle(title: string): void };
    }): void {
        stopTitlebarSpinner(ctx);
        titlebarTimer = setInterval(() => {
            const frame =
                BRAILLE_FRAMES[titlebarFrameIndex % BRAILLE_FRAMES.length];
            ctx.ui.setTitle(`${frame} ${getTitleBase()}`);
            titlebarFrameIndex++;
        }, 80);
    }

    function stopTitlebarSpinner(ctx: {
        ui: { setTitle(title: string): void };
    }): void {
        if (titlebarTimer) {
    ...
    function updateWidget(ctx: UiContext): void {
        ...
        for (const job of runningJobs) {
            const duration = formatDuration(Date.now() - job.startTime);
            pills.push(
                `◐ ${job.id}: ${job.command.slice(0, 25)} (${duration})`
            );
        }
    ...
    function startTimeoutTimer(
        ...
        const timer = setTimeout(() => {
            ...
            const job = Array.from(backgroundJobs.values()).find(
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            jobId: Type.Optional(
                Type.String({ description: "Job ID for output/kill/attach" })
            ),
            wait: Type.Optional(
                Type.Boolean({
                    description:
    ...
    async function handleBackgroundShortcut(ctx: UiContext): Promise<void> {
        ...
        if (didBackgroundBash) {
            ctx.ui.notify(
                "\u23f8 Backgrounded bash + agent. Ctrl+B to resume.",
                "info"
            );
        } else {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            jobId: Type.String({ description: "The job ID to decide on" }),
        ...
        async execute(
            ...
            switch (params.decision) {
                ...
                case "check": {
                    ...
                    // Don't clear pending — agent still needs to decide after checking.
                    return {
                        content: [
                            {
                                type: "text",
    ...
    pi.on("tool_call", async (_event): Promise<ToolCallEventResult> => {
        ...
        if (
            ...
            return {
                block: true,
                reason: `A background job (${pendingDecisionJobId}) is awaiting your decision (${status}). Use job_decide or jobs first.`,
            };
        }
    ...
    // Restart the elapsed timer on each turn.
...
After completing a step, include a [DONE:n] tag in your response.`,
    ...
    async function showTasksInterface(ctx: UiContext): Promise<void> {
        ...
        for (const job of runningJobs) {
            const duration = formatDuration(Date.now() - job.startTime);
            items.push(
                `\u25d0 ${job.id}: ${job.command.slice(0, 40)} \u00b7 ${duration}`
            );
        }
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            switch (params.action) {
                ...
                case "clear": {
                    ...
                    return {
                        content: [
                            { type: "text", text: `Cleared ${count} todos` },
                        ],
                        details: {
                            action: "clear",
                            todos: [],
        ...
        render(width: number): string[] {
            ...
            if (this.list.length === 0) {
                ...
                for (const todo of this.list) {
                    ...
                    lines.push(
                        truncateToWidth(`  ${check} ${id} ${text}`, width)
                    );
                }
            }
    ...
    pi.registerCommand("todos", {
        ...
        handler: async (_args, ctx) => {
            ...
            await ctx.ui.custom<void>((_tui, theme, _kb, done) => {
                return new TodoListComponent(todos, theme, () => {
                    done();
                });
            });
        },
    ...
    pi.on("session_start", async (_event, ctx) => {
        ...
        if (planModeEntry?.data) {
            ...
            planExecutionMode =
                planModeEntry.data.executing ?? planExecutionMode;
        }
    ...
    pi.on("session_shutdown", async (_event, ctx) => {
        ...
        pi.appendEntry("background-tasks-state", {
            jobs: Array.from(backgroundJobs.entries()).map(([id, job]) => [
                id,
                {
                    ...job,
                    proc: undefined,
